...
func main() {
	...
	endpoint, err := zipkin.NewEndpoint("srv5", "host.docker.internal:65516")
	if err != nil {
	...
	serverMiddleware := zipkinhttp.NewServerMiddleware(
		...
		zipkinhttp.SpanName("serve srv5"),
	)

	f := &Foo{}
	http.Handle("/test", serverMiddleware(f))
	http.ListenAndServe(":65516", nil)
}
...
func (f *Foo) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("response from srv5"))
}